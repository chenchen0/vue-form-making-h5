<template>
  <el-container>
    ...
    >
      <generate-form v-if="previewVisible" :data="widgetForm" :remote="remoteFuncs" :value="widgetModels" ref="generateForm">
        <template slot="blank" slot-scope="scope">
          宽度：<el-input v-model="scope.model.blank.width" style="width: 100px"></el-input> 高度：<el-input
    ...
    <!-- 导入json -->
    <cus-dialog :visible="importJsonVisible" @on-close="importJsonVisible = false" ref="jsonPreview" width="800px" form>
      <p>JSON格式如下，直接复制生成的json覆盖此处代码点击确定即可</p>
      <div id="jsoneditor2" style="height: 400px; width: 100%">
        {{ importJsonTemplate }}
      </div>
